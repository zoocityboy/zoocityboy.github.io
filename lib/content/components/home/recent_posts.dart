...
class RecentPostsSection extends StatelessComponent {
  ...
  Component build(BuildContext context) {
    ...
    posts.sort((a, b) {
      final DateTime da =
          DateTime.tryParse((a.data['page'] is Map ? (a.data['page'] as Map)['date']?.toString() : null) ?? '') ??
          DateTime.fromMillisecondsSinceEpoch(0);
      final DateTime db =
          DateTime.tryParse((b.data['page'] is Map ? (b.data['page'] as Map)['date']?.toString() : null) ?? '') ??
          DateTime.fromMillisecondsSinceEpoch(0);
      ...
      div(classes: 'grid gap-3 md:grid-cols-3', [
        if (recent.isEmpty)
          div(classes: 'pb-card p-6', [
            p(classes: 'text-sm text-muted-foreground', [.text('No posts yet.')]),
          ])
        else
          ...recent.map((Page post) {
            final Object? postData = post.data['page'];
            final Map<String, dynamic> meta = postData is Map
                ? postData.map((k, v) => MapEntry(k.toString(), v))
                : <String, dynamic>{};
            ...
            return article(classes: 'pb-card p-6', [
              ...
              if (description.isNotEmpty)
                p(classes: 'mt-3 text-sm leading-6 text-muted-foreground', [Component.text(description)]),
            ]);
          }),
      ]),
    ]);
